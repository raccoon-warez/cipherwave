...
// Automatic server discovery with enhanced error handling
async function discoverServer(roomId) {
    ...
    for (const server of knownServers) {
        try {
            ...
            // Use enhanced signaling with retry mechanism
            const enhancedSignaling = new EnhancedSignaling(3, 2000);
            const socket = await enhancedSignaling.connectWithRetry(server, roomId);
            // Close the test connection and use the original signaling
            socket.close();
    ...
    // If no known servers work, try fallback servers
    try {
        updateConnectionStatus('Trying fallback servers...', 'status-disconnected');
        const fallbackServer = await findWorkingServer(roomId);
        await signaling.connect(fallbackServer, roomId);
        return fallbackServer;
    } catch (err) {
        console.error('Failed to connect to fallback servers:', err.message);
    }
...
function generateRoomId() {
    // Use enhanced security module if available
    if (typeof CipherWaveSecurity !== 'undefined' && CipherWaveSecurity.SecureRandom) {
        return CipherWaveSecurity.SecureRandom.generateSecureRoomId();
    }
...
function updateConnectionStatus(text, className) {
    ...
    // Update peer status in chat header for Telegram UI
    if (peerStatus) {
...
function sendMessage() {
    ...
    try {
        ...
        const startTime = window.performanceMonitor ? window.performanceMonitor.startTimer('encryptionTime') : null;
        ...
        if (window.performanceMonitor && startTime) {
            window.performanceMonitor.endTimer('encryptionTime', startTime);
        }
